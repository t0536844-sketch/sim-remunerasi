...
class Config:
    ...
    f"{project_name}/database/schema.sql": """-- Tabel Pegawai
...
CREATE TABLE IF NOT EXISTS hasil_remunerasi (
    ...
    f"{project_name}/app.py": """from flask import Flask, render_template, request, jsonify, redirect, url_for, flash, send_file
...
class IndexingPegawai(db.Model):
    ...
    kompetensi = db.Column(db.Float, default=0)
    risk = db.Column(db.Float, default=0)
    emergency = db.Column(db.Float, default=0)
...
@app.route('/')
def dashboard():
    total_pendapatan = db.session.query(db.func.sum(PendapatanJasa.total_pendapatan)).scalar() or 0
    return render_template('dashboard.html', total_pendapatan=total_pendapatan)
...
def hitung_langsung():
    ...
    for i in items:
        ...
        insentif = jasa * 0.6 * koef
        hasil.append({'pegawai_id': i['pegawai_id'], 'insentif_langsung': round(insentif, 2)})
...
@app.route('/export/excel/<int:periode_id>')
def export_excel(periode_id):
    # Implementasi export Excel (bisa dikembangkan)
    return jsonify({'message': 'Fitur export dalam pengembangan'})
...
<body>
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item"><a class="nav-link" href="/input_pendapatan">Input Pendapatan</a></li>
                    <li class="nav-item"><a class="nav-link" href="/input_jasa">Input Jasa</a></li>
    ...
    <footer class="text-center py-3 text-muted small">
        &copy; 2024 BLUD RSUD Mimika | Perbup No. 32/2023
    </footer>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script src="{{ url_for('static', filename='js/calc_engine.js') }}"></script>
</body>
</html>""",

    f"{project_name}/templates/dashboard.html": """{% extends "base.html" %}
{% block content %}
<div class="row">
    <div class="col-12 mb-4">
        <h2>📊 Dashboard Remunerasi</h2>
        <p class="text-muted">Periode: {{ month_name }} {{ year }}</p>
    </div>
</div>
<div class="row g-3">
    <div class="col-md-3">
        <div class="card bg-primary text-white">
            <div class="card-body">
                <h6>Total Pendapatan Jasa</h6>
                <h3>Rp {{ "{:,.0f}".format(total_pendapatan) }}</h3>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card bg-success text-white">
            <div class="card-body">
                <h6>Dana Remunerasi (35%)</h6>
                <h3 id="dana-remun">Rp 0</h3>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card bg-warning text-dark">
            <div class="card-body">
                <h6>Insentif Langsung (60%)</h6>
                <h3 id="insentif-langsung">Rp 0</h3>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card bg-info text-white">
            <div class="card-body">
                <h6>Pos Remunerasi (40%)</h6>
                <h3 id="pos-remun">Rp 0</h3>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/api/hitung_alokasi', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({total_pendapatan: totalPendapatan})
    })
    .then(res => res.json())
    .then(data => {
        ...
        new Chart(document.getElementById('chartAlokasi'), {
            type: 'doughnut',
            data: {
                labels: ['Operasional', 'Remunerasi'],
                datasets: [{data: [data.dana_operasional, data.dana_remunerasi], backgroundColor: ['#6c757d', '#198754']}]
            }
        });
        new Chart(document.getElementById('chartDistribusi'), {
            type: 'pie',
            data: {
                labels: ['Langsung (60%)', 'Tidak Langsung (40%)'],
                datasets: [{data: [data.insentif_langsung, data.pos_remunerasi], backgroundColor: ['#ffc107', '#0dcaf0']}]
            }
        });
    });
});
...
{% endblock %}""",
    ...
    f"{project_name}/templates/indexing.html": """{% extends "base.html" %}
{% block content %}
<h2>🎯 Input Indexing Pegawai</h2>
<p class="text-muted">Periode: <strong>{{ bulan }} {{ tahun }}</strong></p>

<form id="formIndexing">
    <div class="table-responsive">
        <table class="table table-bordered table-hover">
            <thead class="table-light">
                <tr>
                    <th>Pegawai</th>
                    <th>Basic<br><small class="text-muted">Gaji/500rb</small></th>
                    <th>Kompetensi</th>
                    <th>Risk</th>
                    <th>Emergency</th>
                    <th>Position</th>
            ...
            <tbody id="tbodyIndexing">
                <!-- Diisi via JS -->
            </tbody>
        </table>
    </div>
    <button type="button" class="btn btn-primary" onclick="hitungIndexing()">💰 Hitung Insentif Tidak Langsung</button>
    <button type="button" class="btn btn-success" onclick="simpanHasil()">💾 Simpan Hasil</button>
</form>

<div id="hasilIndexing" class="mt-4 d-none">
    <h4>📋 Hasil Perhitungan</h4>
    <div class="table-responsive">
        <table class="table table-striped">
            <thead><tr><th>Pegawai</th><th>Total Score</th><th>Insentif Tidak Langsung</th></tr></thead>
...
<script>
const pegawaiList = {{ pegawai_json | safe }}; // Dari backend
const posRemunerasi = {{ pos_remunerasi }}; // Dari backend

function renderForm() {
    const tbody = document.getElementById('tbodyIndexing');
    tbody.innerHTML = '';
    pegawaiList.forEach(p => {
        const basic = Math.floor(p.gaji_pokok / 500000);
        tbody.innerHTML += `
            <tr data-id="${p.id}">
                <td>${p.nama}<br><small class="text-muted">${p.jabatan}</small></td>
                <td><input type="number" class="form-control score-input" data-field="basic" value="${basic}" min="0"></td>
                <td><input type="number" class="form-control score-input" data-field="kompetensi" value="0" min="0"></td>
                <td><input type="number" class="form-control score-input" data-field="risk" value="0" min="0"></td>
                <td><input type="number" class="form-control score-input" data-field="emergency" value="0" min="0"></td>
                <td><input type="number" class="form-control score-input" data-field="position" value="0" min="0"></td>
                <td><input type="number" class="form-control score-input" data-field="performance" value="0" min="0" max="10"></td>
                <td class="total-score fw-bold">0</td>
            </tr>`;
    });
    // Auto-calc total score on input
    document.querySelectorAll('.score-input').forEach(inp => {
        inp.addEventListener('input', calcRowScore);
    });
}
function calcRowScore(e) {
    const row = e.target.closest('tr');
    let total = 0;
    row.querySelectorAll('.score-input').forEach(inp => total += parseFloat(inp.value) || 0);
    row.querySelector('.total-score').textContent = total.toFixed(2);
}
async function hitungIndexing() {
    const items = [];
    document.querySelectorAll('#tbodyIndexing tr').forEach(row => {
        const scores = {};
        row.querySelectorAll('.score-input').forEach(inp => scores[inp.dataset.field] = parseFloat(inp.value) || 0);
        items.push({pegawai_id: row.dataset.id, scores});
    });
    const res = await fetch('/api/hitung_indexing', {
        method: 'POST', headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({items, pos_remunerasi: posRemunerasi})
    });
    const hasil = await res.json();
    const tbody = document.getElementById('tbodyHasil');
    tbody.innerHTML = '';
    hasil.forEach(h => {
        const p = pegawaiList.find(x => x.id == h.pegawai_id);
        tbody.innerHTML += `<tr><td>${p.nama}</td><td>${h.total_score}</td><td>Rp ${h.insentif_tidak_langsung.toLocaleString('id-ID')}</td></tr>`;
    });
    document.getElementById('hasilIndexing').classList.remove('d-none');
}
function simpanHasil() {
    alert('✅ Data berhasil disimpan (implementasi backend diperlukan)');
}
document.addEventListener('DOMContentLoaded', renderForm);
</script>
{% endblock %}""",

    f"{project_name}/static/css/style.css": """body { background-color: #f8f9fa; font-family: 'Segoe UI', sans-serif; }
...
footer { border-top: 1px solid #dee2e6; margin-top: 3rem; }""",
    ...
    f"{project_name}/static/js/calc_engine.js": """// calc_engine.js - Helper functions for remuneration calculations
window.RemunCalculator = {
    ...
    validateInput: function(data) {
        if (!data.total_pendapatan || data.total_pendapatan <= 0) return false;
        return true;
    }
};""",

    f"{project_name}/docs/README.md": """# 💰 SIM-REMUNERASI BLUD RSUD MIMIKA
